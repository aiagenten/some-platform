...
export async function GET(request: NextRequest) {
  ...
  const redirectUri = `${APP_URL}/api/auth/facebook?action=callback`
  const scopes = [
    'pages_show_list',
  ...
  // Pass org_id through state parameter
...
async function handleCallback(request: NextRequest) {
  ...
  try {
    ...
    if (tokenData.error) {
      ...
      return NextResponse.redirect(`${APP_URL}/dashboard/settings?error=token_exchange_failed`)
    }
    ...
    if (longLivedData.error) {
      ...
      return NextResponse.redirect(`${APP_URL}/dashboard/settings?error=long_lived_token_failed`)
    }
    ...
    if (pagesData.error) {
      ...
      return NextResponse.redirect(`${APP_URL}/dashboard/settings?error=pages_fetch_failed`)
    }
    ...
    // Store each page's access token
    ...
    for (const page of pages) {
      ...
      if (page.instagram_business_account) {
        ...
        await supabase.rpc('upsert_social_account', {
          ...
          p_access_token: page.access_token, // Use Page Access Token for IG
          p_token_expires_at: new Date(Date.now() + expiresIn * 1000).toISOString(),
          p_token_secret: tokenSecret,
    ...
    return NextResponse.redirect(`${APP_URL}/dashboard/settings?error=unknown_error`)
  }
}
